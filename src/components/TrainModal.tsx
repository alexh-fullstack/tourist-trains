...
export const TrainModal: React.FC<TrainModalProps> = ({
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="relative bg-white w-full max-w-3xl rounded-xl shadow-2xl border border-gray-200 overflow-hidden my-auto max-h-[92vh] flex flex-col transform transition-all"
      >
        ...
        {/* Scrollable Content */}
        <div className="p-5 sm:p-7 overflow-y-auto space-y-6 flex-1 text-gray-800">
          {/* Full Description */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
              Описание туристического маршрута
            </h4>
            <p className="text-sm sm:text-base text-gray-700 leading-relaxed bg-gray-50 p-4 rounded-lg border border-gray-200">
              {train.description}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 bg-gray-50 border border-gray-200 p-3 sm:p-4 rounded-lg">
              {train.route.map((city, idx) => (
                <React.Fragment key={idx}>
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 rounded-md text-xs sm:text-sm font-semibold text-gray-900 shadow-xs">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {train.excursions.map((excursion, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-2.5 p-3 rounded-lg bg-emerald-50/50 border border-emerald-200 text-emerald-950 text-xs sm:text-sm"
                >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {train.departures.map((dateStr, idx) => (
                <div
                  key={idx}
                  className="p-3 rounded-lg bg-gray-50 border border-gray-200 text-center flex flex-col items-center justify-center gap-1 hover:border-[#E21A1A] transition"
                >
                  <Calendar className="w-4 h-4 text-gray-400" />
                  <span className="text-xs font-bold text-gray-900">{formatDate(dateStr)}</span>
              ...
              <div className="flex flex-wrap gap-2">
                {train.tags.map((tag) => (
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs font-medium bg-gray-100 hover:bg-red-50 hover:text-[#E21A1A] text-gray-700 px-3 py-1 rounded transition cursor-pointer"
                  >
        ...
        <div className="p-4 sm:p-6 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left">
            <div className="text-xs text-gray-500 font-medium">Стоимость билета за тур</div>
            <div className="text-2xl sm:text-3xl font-extrabold text-gray-900 flex items-baseline justify-center sm:justify-start gap-1.5">
              <span className="text-sm font-normal text-gray-500">от</span>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              type="button"
              onClick={onClose}
              className="hidden sm:inline-flex px-4 py-3 rounded-lg border border-gray-300 hover:bg-gray-100 text-gray-700 text-sm font-semibold transition cursor-pointer"
            >
              Закрыть
            </button>
            <a
